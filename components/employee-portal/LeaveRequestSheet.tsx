...
} from "@/lib/action/employee-leave.action";
...
type Attachment = { name: string; mimeType: "application/pdf" | "image/jpeg" | "image/png" | "image/webp"; size: number; data: string };
...
const leaveTypes: LeaveFormValues["leaveType"][] = [
  "Annual", "Sick", "Emergency", "Maternity", "Paternity", "Without Pay",
];
const supportedFiles = ["application/pdf", "image/jpeg", "image/png", "image/webp"] as const;
...
function readAttachment(file: File): Promise<Attachment> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => resolve({ name: file.name, mimeType: file.type as Attachment["mimeType"], size: file.size, data: String(reader.result) });
...
export default function LeaveRequestSheet({
  ...
  async function onAttachmentChange(file?: File) {
    if (!file) return;
    if (!supportedFiles.includes(file.type as (typeof supportedFiles)[number]) || file.size > 2_000_000) {
      form.setError("reason", { message: "Attachment must be a PDF or image no larger than 2 MB." });
      return;
    }
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Unable to read attachment.");
    }
  }
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        {record ? <Button variant="ghost" size="sm">Edit</Button> : <Button><CalendarPlus /> Request leave</Button>}
      </SheetTrigger>
      <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
        <SheetHeader>
          <SheetTitle>{isEdit ? "Edit pending request" : "Request time away"}</SheetTitle>
          <SheetDescription>Leave balances are checked again before your request is saved.</SheetDescription>
        </SheetHeader>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 p-4">
          <div className="grid grid-cols-2 gap-3 rounded-lg bg-muted/60 p-3 text-sm">
            {balances.filter((item) => ["Annual", "Sick"].includes(item.leaveType)).map((balance) => (
              <div key={balance.leaveType}>
                <p className="text-muted-foreground">{balance.leaveType} available</p>
                <p className="font-semibold">{balance.available ?? "Untracked"}{balance.available === null ? "" : " days"}</p>
              </div>
            ))}
          </div>
          <Field data-invalid={Boolean(form.formState.errors.leaveType)}>
            <FieldLabel>Leave type</FieldLabel>
            <Select value={form.watch("leaveType")} onValueChange={(value) => form.setValue("leaveType", value as LeaveFormValues["leaveType"], { shouldDirty: true })}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field data-invalid={Boolean(form.formState.errors.startDate)}>
              ...
              <Input id="leave-start" type="date" min={new Date().toISOString().slice(0, 10)} {...form.register("startDate")} />
              <FieldError errors={[form.formState.errors.startDate]} />
            </Field>
            <Field data-invalid={Boolean(form.formState.errors.endDate)}>
              <FieldLabel htmlFor="leave-end">End date</FieldLabel>
              <Input id="leave-end" type="date" min={startDate || new Date().toISOString().slice(0, 10)} {...form.register("endDate")} />
              <FieldError errors={[form.formState.errors.endDate]} />
            </Field>
          </div>
          <p className="rounded-lg border p-3 text-sm"><span className="font-medium">Estimated duration: </span>{estimatedDays ? `${estimatedDays} day${estimatedDays === 1 ? "" : "s"}` : "Select valid dates"}</p>
          <Field data-invalid={Boolean(form.formState.errors.reason)}>
            <FieldLabel htmlFor="leave-reason">Reason</FieldLabel>
            <Textarea id="leave-reason" placeholder="Share enough context for your approver." {...form.register("reason")} />
            <FieldError errors={[form.formState.errors.reason]} />
          </Field>
          <Field>
            <FieldLabel htmlFor="leave-attachment">Attachment (optional)</FieldLabel>
            <Input id="leave-attachment" type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(event) => onAttachmentChange(event.target.files?.[0])} />
            <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground"><Paperclip className="size-3" /> {attachment?.name ?? "PDF or image up to 2 MB."}</p>
          </Field>
